...
export default function Header() {
  ...
  return (
    <header className=" sticky top-0 p-6 bg-white border-b border-solid border-blue-900 shadow-md z-50 text-2xl sm:text-3xl md:text-4xl sm:p-8 flex items-center justify-between">
      {openModal && <Modal />}
      <Link href={"/"}>
        <h1 className="uppercase cursor-pointer hover:scale-105 hover:font-medium ">
          Fruit Shop
        </h1>
      </Link>
      <div
      ...
      >
        {cartItems.length > 0 && (
          <div className="absolute aspect-square pointer-events-none h-5 sm:h-6 grid place-items-center top-0 bg-red-400 text-white rounded-full right-0 -translate-y-1/2 translate-x-1/2">
